...
interface LiquidTextProps {
    ...
    /** Text alignment inside the texture */
    textAlign?: CanvasTextAlign;
    /** Multiline spacing multiplier */
    lineHeight?: number;
    /** Plane rotation in radians */
    rotation?: number;
    /** Scale text up or down to occupy the available texture area */
    fitToCanvas?: boolean;
...
const createTextTexture = (
    ...
    canvas.width = Math.max(1024, Math.round(2048 * aspect));
    canvas.height = 2048;
    ...
    if (fitToCanvas) {
        ctx.font = `${fontWeight} ${activeSize}px ${font}`;
        const widestLine = Math.max(...lines.map((line) => ctx.measureText(line).width), 1);
        const totalHeight = activeSize + (lines.length - 1) * activeSize * lineHeight;
        const maxWidth = canvas.width * 0.84;
        const maxHeight = canvas.height * 0.72;
        activeSize = activeSize * Math.min(maxWidth / widestLine, maxHeight / totalHeight);
    }

    const lineGap = activeSize * lineHeight;
    ...
    lines.forEach((line, index) => {
        ctx.fillText(line, x, startY + index * lineGap);
    });
...
export function LiquidText({
    ...
    useEffect(() => {
        ...
        const geometry = new THREE.PlaneGeometry(15 * aspect, 15, 100, 100);
    ...
    }, [text, fontSize, font, fontWeight, textAlign, lineHeight, rotation, fitToCanvas, color, lightColor, darkColor]);
