...
export function HeroSection() {
  ...
  const heroScale        = useTransform(smooth, [0, 1], [1, 0.82]);
  const heroOpacity      = useTransform(smooth, [0, 0.78], [1, 0]);
  const heroBorderRadius = useTransform(smooth, [0, 0.6], [0, 24]);
  const imageScale       = useTransform(smooth, [0, 1], [1, 1.06]);
  const imageY           = useTransform(smooth, [0, 1], ["0%", "4%"]);
        ...
        >
          ...
          >
            ...
            {[
              ...
              <div key={i} style={{ position: "absolute", inset: 0, zIndex: 2, pointerEvents: "none", ...style }} />
            ))}
          </motion.div>
          ...
          >
            <span style={{ fontFamily: "var(--font-content)", fontSize: "0.58rem", fontWeight: 600, letterSpacing: "0.38em", textTransform: "uppercase", color: "inherit" }}>
              Scroll
            </span>
            <motion.svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden
              animate={{ y: [0, 4, 0] }}
